import { useRouter } from "next/router";
import { useEffect, useState } from "react";
import { getEmployeeList } from "@/app/api/requesters";

const EmployeeList = () => {
  const router = useRouter();

  // Employees
  const [employeesError, setEmployeesError] = useState(null);
  const [employees, setEmployee] = useState([]);
  const [filters, setFilters] = useState({});
  const [shoudlFetchEmployees, setShouldFetchEmployees] = useState(true);

  useEffect(() => {
    if (shoudlFetchEmployees) {
      fetchEmployees(filters);
      setShouldFetchEmployees(false);
    }
  }, [shoudlFetchEmployees]);

  const fetchEmployees = async (filters) => {
    try {
      const response = await getEmployeeList(filters);
      if (response.ok) {
        const data = await response.json();
        setEmployee(data);
      } else {
        const status = response.status;
        const statusText = response.statusText;
        throw new Error(`Request failed with status: ${status} ${statusText}`);
      }
    } catch (error) {
      setEmployeesError(error);
    }
  };

  const handlerowClick = (employeeID) => {
    router.push(`/employees/${employeeID}`);
  };

  const handleApplyFilters = () => {
    setShouldFetchEmployees(true);
  };

  const handleFormChange = (e) => {
    const { name, value } = e.target;
    let updatedValue = value;
    
    if (value === "--SELECT--") {
      updatedValue = "";
    }

    setFilters((prevFiltersData) => ({
      ...prevFiltersData,
      [name]: updatedValue,
    }));
  };

  const filtersForm = () => {
    return (
      <div className="p-4 border rounded shadow bg-gray-100">
        <h2 className="font-semibold text-lg mb-4">Filters</h2>
        <form className="mb-4" onChange={handleFormChange}>
          <label className="block mb-2 font-medium">First Name</label>
          <input
            type="text"
            name="first_name"
            placeholder="E.g. John"
            className="w-full p-2 border rounded focus:outline-none mb-2"
          />
          <label className="block mb-2 font-medium">Last Name</label>
          <input
            type="text"
            name="last_name"
            placeholder="E.g. Smith"
            className="w-full p-2 border rounded focus:outline-none mb-2"
          />
          <label className="block mb-2 font-medium">ID</label>
          <input
            min="0"
            type="number"
            name="id_card"
            placeholder="E.g. 1234567"
            className="w-full p-2 border rounded focus:outline-none mb-2"
          />
        </form>
        <button
          onClick={handleApplyFilters}
          className="bg-[#6e8a85] text-white font-semibold p-2 rounded w-full"
        >
          Apply Filters
        </button>
      </div>
    )
  }

  const employeesTableHead = () => {
    return (
      <thead>
        <tr className="bg-gray-100">
          <th className="py-2 px-4 border-b text-center w-1/9">First Name</th>
          <th className="py-2 px-4 border-b text-center w-2/9">Last Name</th>
          <th className="py-2 px-4 border-b text-center w-1/9">ID Card</th>
          <th className="py-2 px-4 border-b text-center w-1/9">Email</th>
          <th className="py-2 px-4 border-b text-center w-1/9">Phone</th>
          <th className="py-2 px-4 border-b text-center w-1/9">Is Employed</th>
        </tr>
      </thead>
    );
  };

  const employeesTableBody = () => {
    return (
      <tbody>
        {employees.length > 0 ? (
          employees.map((employee, index) => (
            <tr
              key={index}
              className="hover:bg-[#d3d3d3]"
              onClick={() => handlerowClick(employee.id)}
            >
              <td className="py-2 px-4 border-b text-center w-1/9">{employee.first_name}</td>
              <td className="py-2 px-4 border-b text-center w-1/9">{employee.last_name}</td>
              <td className="py-2 px-4 border-b text-center w-1/9">{employee.id_card}</td>
              <td className="py-2 px-4 border-b text-center w-1/9">{employee.email}</td>
              <td className="py-2 px-4 border-b text-center w-1/9">{employee.phone}</td>
              <td className="py-2 px-4 border-b text-center w-1/9">{employee.is_employed ? "True" : "False"}</td>
            </tr>
          ))
        ) : (
          <tr>
            <td colSpan={12} className="py-2 px-4 text-center w-1/9">
              No employees found.
            </td>
          </tr>
        )}
      </tbody>
    );
  };

  return (
    <div className="p-4">
      <h2 className="text-center text-2xl font-bold">Employees</h2>
      <br />
      {employeesError ? (
        <div>
          <p className="text-center">{`${employeesError}`}</p>
        </div>
      ) : (
        <div className="grid grid-cols-6 gap-4">
          <div className="col-span-5 border rounded">
            <table className="w-full border-collapse">
              {employeesTableHead()}
              {employeesTableBody()}
            </table>
          </div>
          <div>
            {filtersForm()}
          </div>
        </div>
      )}
    </div>
  );
};

export default EmployeeList;